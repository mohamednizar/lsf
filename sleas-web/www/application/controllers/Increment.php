...
class Increment extends CI_Controller {
    ...
    public function calculateSalary($grade, $salary_step, $increment_year){
        ...
        if($grade == 'Grade I'){
            ...
            $salary = ($grsp_start + ($grsp_start_incr * ($salary_step - 1))) + ((($grsp_end + ($grsp_end_incr * ($salary_step - 1))) - ($grsp_start + ($grsp_start_incr * ($salary_step - 1))))/5)*($increment_year - 2015);
    ...
    public function increment_add()
    {
        ...
        $increment_date = $this->security->xss_clean($_REQUEST['increment_date']);
